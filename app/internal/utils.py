...
class LSTM_model_train:
    ...
    def save(self) -> None:
        ...
        _mse, _mae, _rmse_error = self.model.evaluate(self.X_test, self.y_test)
        _tag = f'time_step={self.config.time_step}&epoch={self.config.epochs}&batch_size={self.config.batch_size}&learning_rate={self.config.learning_rate}&nn_activation={self.config.nn_activation}&nn_max_units={self.config.nn_max_units}&MSE={str(_mse)}&MAE={str(_mae)}&RMSE={str(_rmse_error)}'
